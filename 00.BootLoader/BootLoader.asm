...
READDATA:
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    add si, 0x0020                    ; 512(0x200)바이트만큼 읽었으로
                                      ; 이를 세그먼트 레지스터로 설정
    ...
    ; 섹터 번호를 1로 설정
    xor byte [ HEADNUMBER ], 0x01     ; 헤드 번호를 0x01 과 XOR하여 토글
    mov byte [ SECTORNUMBER ], 0x01   ; 섹터 번호를 다시 1로 설정

    ; 만약 헤드가 1->0로 바뀌었다면 양쪽 헤드를 모두 읽은 것이므로 아래로 이동하여
    ; 트랙 번호를 1 증가
...
.MESSAGEEND:
    pop dx                            ; 함수에서 사용이 끝난 레지스터를 스택에서 제거
    pop cx
    pop ax
    pop di
    pop si
