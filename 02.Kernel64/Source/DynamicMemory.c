...
static int kCalculateMetaBlockCount( QWORD qwDynamicRAMSize )
{
  ...
  // 할당된 블록이 속한 리스트의 인덱스를 저장하는데 필요한 영역을 계산
...
void * kAllocateMemory( QWORD qwSize )
{
  ...
  // 블록 크기를 저장하는 여역에 실제로 할당된 버디 블록이 속한 블록 리스트의 인덱스를 저장
...
// 블록 리스트의 비틈맵을 검색한 후 블록이 존재하면 블록의 오프셋을 반환
static int kFindFreeBlockInBitmap( int iBlockListIndex )
{
  ...
  for( i = 0; i < iMaxCount; )
  {
    // QWORD는 8 * 8 비트 => 64비트이므로, 64비트를 한꺼번에 검사해서 1인 비트가 있는지 확인
    if( ( ( iMaxCount - i ) / 64 ) > 0 )
    {
      pqwBitmap = ( QWORD * ) &( pbBitmap[i/8] );
      // 만약 8바이트 모두 0이면 8바이트 모두 제외
      if( *pqwBitmap == 0 )
      {
        i += 64;
        continue;
...
static BOOL kFreeBuddyBlock( int iBlockListIndex, int iBlockOffset )
{
  ...
  // 블록 리스트의 끝까지 인접한 블록을 검사하여 결함할 수 없을 때까지 반복
  for( i = iBlockListIndex; i < gs_stDynamicMemory.iMaxLevelCount; i++ )
  {
    // 현재 블록은 존재하는 상태로 ㅅ러정
    kSetFlagInBitmap( i, iBlockOffset, DYNAMICMEMORY_EXIST );

    // 블록의 오프셋이 짝수면 홀수를 검사하고, 홀수면 짝수의
