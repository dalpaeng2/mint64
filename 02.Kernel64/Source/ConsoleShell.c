...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
  ...
  { "settimer", "Set PIT Controller Counter, ex)settimer 10(ms) 1(periodic)",
              kSetTimer },
  ...
  { "killtask", "End Task, ex)killtask 1(ID) or 0xffffffff(All Taks)", kKillTask },
  ...
  { "writesector", "Wrtie HDD Sector, ex)writesector 0(LBA) 10(count)", kWriteSector },
...
static void kChangeTaskPriority( const char * pcParameterBuffer )
{
  ...
  kPrintf( "Chage Task Priority ID [0x%q] Priority[%d] ", qwID, bPriority );
...
static void kPrintNumberTask( void )
{
  ...
  for( i = 0; i < 5; i++ )
  {
    ...
    for( j = 0; j < 30000; j++ ) ; // Keep CPU Busy
  }
...
static void kMountHDD( const char * pcParameter )
{
  if( kMount() == FALSE )
  {
...
static void kFormatHDD( const char * pcParameter )
{
  if( kFormat() == FALSE )
  {
